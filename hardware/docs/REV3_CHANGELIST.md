...
5. **Gyro LED (D9) fixed.** Previously recorded as a 180 degree mounting
   error. That was never it: **D9, R39 and U15 are not present on the Rev 2
   board at all.** They exist in the Rev 2 schematic but were never placed in
   the Rev 2 layout, which instead carried an unused NCV8187AMT180TAG
   footprint. D4, D5, D7 and their series resistors are all on the Rev 2 board,
   which is why only this one indicator appeared dead.

   The current board has all three placed, DRC reports zero unconnected items
   and schematic parity is clean. D9's anode sits on the LP5912 power good
   output (U15 pin 3, open drain, external pullup required) with R39 6.49k to
   +5 V, so the LED lights when the 1.8 V rail is in regulation and goes out
   when PG asserts low.
...
7. **D9 brightness.** R39 6.49k gives the gyro LED about 0.32 mA against 0.78
   to 0.95 mA for D4, D5 and D7, so it will be noticeably dimmer than the other
   indicators. Dropping R39 to about 2.4k matches D5. The LP5912's PG low-level
   output is characterised at 1 mA sink; 2.4k asks for roughly 2 mA, which only
   has to stay below the LED's turn-on to keep it dark, so there is ample
   margin.

8. **Input capacitors stay at 4.7 uF 50 V** (C22, C23) for 8S. Rated voltage is
...
9. **Inductor headroom.** L2 and L3 stay at 4.7 uH. Ripple at 33.6 V input is
...
10. **VBAT sense divider unchanged** at R1 100k / R2 10k. At 8S this reads
